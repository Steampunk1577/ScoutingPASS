...
2. Add gsCol tag in config file
  ...
  Sheets which column to put the data in.
    * For example, the first entry in our configuraiton is Scouter Name, the new entry with the gsCol tag would look like this:
```json
            { "name": "Scouter Name",
              "gsCol": "scouter",
              "code": "s",
...
```
    * The Scouter Name will be put in the column with the header "scouter"
